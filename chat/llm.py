...
def llm_conversation_title(conversation):
    try:
        conversation_text = "\n".join([msg.message for msg in conversation.messages.order_by("timestamp")])
...
def llm_form_core_memories(conversation, bot):
    ...
    conversation_text = "\n".join([f"{msg.participant.name()}: {msg.message}" for msg in conversation.messages.select_related("participant__user", "participant__bot").order_by("timestamp")])
